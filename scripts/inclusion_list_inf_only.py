...
def make_inclusion_list_classifier(score_file, inf_psm_matches_file, outfile, min_intensity = 1e4, min_score = 0.9, min_rt = 25.*60, max_rt = 115.*60., window = 3.):
    peaks = pd.read_csv(score_file)
    ...
    after_loading = peaks['rt'] > min_rt
    before_washout = peaks['rt'] < max_rt
    inf_snr_threshold = peaks['p_unique'] >= min_score
    no_psm_match = logical_not(peaks['mtb_match'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', help = 'scoring output', required = True)
    ...
    parser.add_argument('--classifier', help = 'use classifier scoring instead of AutoMS', action = 'store_true')

    args = parser.parse_args()
    if args.classifier:
        make_inclusion_list_classifier(args.i, args.m, args.o,
